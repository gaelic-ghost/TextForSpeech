...
Configurable URL, markdown-link, and path handling is planned. The current defaults are deterministic and always on; future work will add one normalization configuration value so callers can choose how aggressively those surfaces are spoken, shortened, preserved, or filtered. A Codex hook-oriented request mode is also planned for hook payloads that mix useful text with metadata that should not be read aloud.
